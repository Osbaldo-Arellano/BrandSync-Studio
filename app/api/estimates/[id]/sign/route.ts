...
export async function POST(
  ...
  const { data: newInvoice, error: invCreateErr } = await admin
    .from("invoices")
    .insert({
  ...
  if (invCreateErr || !newInvoice) {
    console.error("[sign] Auto-invoice creation failed:", invCreateErr?.message);
    // Estimate IS signed — don't fail the whole request. Surface the warning.
    // (PDF + email still send below)
  } else {
    ...
    const invoiceItems = (items ?? []).map((item) => ({
      ...
      line_total: (item.line_total as number) ?? ((item.quantity ?? 0) * (item.unit_price ?? 0)),
    }));
  ...
  return NextResponse.json({
    ok: true,
    invoiceId,
    ...(invoiceId === null ? { warning: "Estimate signed, but invoice could not be created automatically. Please create it manually." } : {}),
  });
  } catch (error) {
