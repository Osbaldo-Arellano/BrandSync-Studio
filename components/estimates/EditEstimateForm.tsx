...
export function EditEstimateForm({
  ...
  const [salesperson, setSalesperson] = useState(estimate.salesperson);
  const [job, setJob] = useState(estimate.job);
  const [paymentTerms, setPaymentTerms] = useState(estimate.payment_terms || "End of project");
  const [isTBD, setIsTBD] = useState(estimate.due_date === "TBD" || !estimate.due_date);
  const [dateValue, setDateValue] = useState(estimate.due_date === "TBD" ? "" : (estimate.due_date ?? ""));
  const [items, setItems] = useState<FormLineItem[]>(
    estimate.items.length > 0
      ? estimate.items.map((i) => ({ id: i.id, description: i.description, quantity: i.quantity, unit_price: i.unit_price, line_total: i.line_total }))
      : [newLineItem()]
  );
  const [deposit, setDeposit] = useState(estimate.deposit);
  ...
  const [cashNote, setCashNote] = useState(estimate.cash_note);
          ...
          <div className="p-5 sm:p-8 md:p-10 space-y-6">
            ...
            <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-2 pb-4 border-b border-gray-200">
              ...
              <div className="sm:text-right flex flex-col items-end gap-1">
                ...
                <p className="text-sm text-gray-500">Date: {new Date(estimate.created_at).toLocaleDateString("en-US")}</p>
              </div>
            </div>
