...
export function EstimateDetail({ estimate, tenant, existingInvoice }: Props) {
  ...
  const [deliveryMethod, setDeliveryMethod] = useState(estimate.delivery_method ?? null);
          ...
          <div className="w-full max-w-md rounded border border-gray-200 bg-white p-6 shadow-xl">
            {sendSuccess ? (
              <div className="flex flex-col items-center gap-3 py-4 text-center">
                ...
                <div>
                  <p className="text-sm font-semibold text-gray-900">Estimate sent</p>
        ...
        <div className="rounded border border-gray-200 bg-white px-5 py-3 flex items-center gap-3">
          {deliveryMethod === "print" ? (
            <svg className="h-4 w-4 text-gray-500 shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
              <path strokeLinecap="round" strokeLinejoin="round" d="M17 17H7V7h10v10zM4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1M4 8V7a3 3 0 013-3h10a3 3 0 013 3v1" />
            </svg>
          ) : deliveryMethod === "link" ? (
            <svg className="h-4 w-4 text-gray-500 shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
          ...
          <p className="text-sm text-gray-500">
            {deliveryMethod === "print" ? "Delivered" : "Sent"}{" "}
            {new Date(sentAt).toLocaleDateString("en-US", { month: "short", day: "numeric", year: "numeric" })}
            {deliveryMethod === "print" && <> · <span className="text-gray-700">physical delivery</span></>}
            {deliveryMethod === "link" && <> · <span className="text-gray-700">shared link</span></>}
            {savedEmail && deliveryMethod !== "print" && deliveryMethod !== "link" && <> · <span className="text-gray-700">{savedEmail}</span></>}
          </p>
        ...
        <div className="rounded border border-gray-200 bg-white px-5 py-4 space-y-3">
          ...
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
            ...
            <div className="flex shrink-0 flex-wrap gap-2">
              ...
              <button
                onClick={() => {
                  window.location.href = `/api/estimates/${estimate.id}/pdf`;
                  setShowDeliverPrompt(true);
                }}
                className="flex items-center gap-1.5 rounded border border-gray-200 px-3 py-2 text-sm font-medium text-gray-600 hover:bg-gray-50 transition-colors"
              >
